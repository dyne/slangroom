...
import type {
	LdpVcElementType,
	LdpVcVerifiableCredentialelementType
} from './types.js';

const resolveDid = async(did: string) => {
	if (!did.startsWith('did:')) throw new Error('Invalid presentation: verificationMethod must be a did');
	...
	if (!response.ok) throw new Error(`Invalid presentation: can not resolve ${did} with ${resolutionUrl}`);
	let didDocument = (await response.json()).didDocument;
	const verificationMethod = didDocument.verificationMethod.find((v: Record<string, unknown>) => v['id'] === did);
	if (!verificationMethod) throw new Error(`Invalid presentation: verificationMethod matching ${did} not found in the did document`);
...
const rdfValidation = async(vp: LdpVcElementType | LdpVcVerifiableCredentialelementType) => {
	const { proof: { proofValue, ...proofWithoutProofValue }, ...vpWithoutProof } = vp;
	(proofWithoutProofValue as typeof proofWithoutProofValue & { '@context'?: unknown })['@context'] = JSON.parse(JSON.stringify(vpWithoutProof['@context']));
