...
import {
	executeDownloadExtract,
	executeListDirectoryContent,
	executeReadFileContent,
	executeStoreInFile,
} from '@slangroom/fs';
...
test.serial('readFileContent works', async (t) => {
	...
	const res = await executeReadFileContent(ctx);
	t.deepEqual(res, {
		ok: true,
		value: content,
	});
...
test.serial('storeInFile works', async (t) => {
	...
	const res = await executeStoreInFile(ctx);
	t.deepEqual(res, {
		ok: true,
		value: null,
	});
...
test.serial('listDirectoryContent works', async (t) => {
	...
	const have = await executeListDirectoryContent(ctx);
